...
function main() {
  ...
  if (packageJson.version !== version) fail(`Requested version ${version} does not match package.json.`);
  ...
  const buildFiles = ["dist/server/index.js", "dist/web/index.html"];
  for (const file of buildFiles) {
    if (!existsSync(join(repositoryRoot, file))) fail(`Missing production build file: ${file}`);
  }
  ...
  const temporaryRoot = mkdtempSync(join(tmpdir(), "rew-portable-package-"));
  try {
    ...
    cpSync(join(repositoryRoot, "dist"), join(stageRoot, "dist"), { recursive: true, force: true });
    writeFileSync(
    ...
    const required = [
      ...
      "plugins/runtime-evolution-workbench/.codex-plugin/plugin.json",
  ...
  writeFileSync(
    manifestPath,
    `${JSON.stringify({
      schema_version: "runtime-evolution-workbench.portable-release.v1",
      version,
      commit,
      release_tier: "technical_preview",
      target_platforms: ["linux-x64", "macos-arm64"],
      evidence_scope: {
        lifecycle: "The tag workflow must install, start, inspect, restart, uninstall, and prove absence from this extracted archive on GitHub-hosted Ubuntu and macOS.",
        physical_device: "not_run",
        authenticated_codex_run: "not_run"
      },
      protocol_dependency: packageJson.dependencies["@runcase/interchange"],
